...
#endif
#if 1
/* Changing all upper char to lower char */
...
char* my_strcasestr(const char* str1, const char* str2) {
    ...
    while (*p1_advance) {
        ...
        while (*p1 && *p2 && (my_tolower(*p1) == my_tolower(*p2))) {
            p1++;
            p2++;
        }
...
char* my_strcasestr(const char* str1, const char* str2) {
    ...
    for (i = 0; str1[i] != '\0'; i++) {
        for (j = 0; str2[j] != '\0'; j++) {
            if (str1[i + j] == '\0' || my_tolower(str1[i + j]) != my_tolower(str2[j])) {
...
int main() {
    ...
    char *result = my_strcasestr(str1, str2);
    if (result) {
